...
describe Api2cart do
  context 'when request is successful', vcr: { cassette_name: 'succefful_request' } do
  ...
  context 'when response contains an error', vcr: { cassette_name: 'erroneous_request' } do
    it 'raises an exception with correspondent message' do
      expect(Api2cart::Store.new('wrong', 'even more wrong').product_count).to raise_error('Ololol wrong key')
    end
  end
end
